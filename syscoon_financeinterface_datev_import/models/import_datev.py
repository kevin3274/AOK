...
class ImportDatev(models.Model):
    ...
    def _convert_to_unicode_dict(self, importcsv, import_config, import_struct, errorlist, start_date, end_date):
        ...
        for linecounter, line in enumerate(importliste, start=1):
            if linecounter == headerrow:
                for spalte in line:
                    try:
                        vorlauf['header'].append(spalte.decode(encoding))
                    except:
                        errorlist.append({
                            'line': linecounter,
            ...
            if linecounter > headerrow:
                thisline = []
                for spalte in line:
                    thisline.append(spalte.decode(encoding))
                line = linecounter - 1
    ...
    def create_main_lines(self, line, thismove, partner_id, import_config, move_lines=None):
        ...
        gegenmove = {
            ...
            'name': 'Gegenbuchung',
            'partner_id': partner_id,
            'ecofi_account_counterpart': line['gegenkonto'].id,
        }
        mainmove = {
            'credit': credit,
            'debit': debit,
        ...
        if not line['buschluessel'] or line['buschluessel'] == 'False':
